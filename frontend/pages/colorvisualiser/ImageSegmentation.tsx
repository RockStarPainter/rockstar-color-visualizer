...
const ImageMaskComponent: React.FC<ImageMaskProps> = ({
  ...
  const displayDimensions = useMemo((): Dimensions => {
    ...
    const naturalAspectRatio = naturalDimensions.width / naturalDimensions.height;
  ...
  const convertSrcToFile = useCallback(async (src: string, fileName = "image.jpg"): Promise<File> => {
    try {
  ...
  const adjustColorOpacity = useCallback((color: string, opacity: number): string => {
  ...
  const drawSegmentsOnCanvas = useCallback((allSegments: Segment[]) => {
  ...
  const handleClick = useCallback(async (e: React.MouseEvent<HTMLImageElement>) => {
    ...
    const x = Math.floor((clickX * naturalDimensions.width) / displayDimensions.width);
    const y = Math.floor((clickY * naturalDimensions.height) / displayDimensions.height);
